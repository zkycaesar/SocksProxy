...
public class ProxyProcessor implements Runnable {
	...
	public ProxyProcessor(Selector sel, int id) throws SecurityException, IOException {
		threadID = id;
...
//					System.out.println("Remote host " + remoteHost + " is unresolved.");
					logger.info("Remote host " + remoteHost + " is unresolved.");
					return;
				}
...
//						"\r\nWriting " + writeCount + " bytes to: " + remoteSocket.getRemoteAddress());
				logger.info("Get message from: " + readSocAddr + ". Byte count: " + readByteCount + 
						"\r\nWriting " + writeCount + " bytes to: " + remoteSocket.getRemoteAddress());
				return;
			}
			else{
